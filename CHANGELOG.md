...
- Testing: migrated `phpunit.xml` to the PHPUnit 13.3 schema. The constraint is `phpunit/phpunit: ^13`, so a plain `composer install` moved the suite from 13.2 to 13.3, where the runner started reporting two issues and exiting with code `1` on an otherwise green suite : the `cacheResult` attribute is deprecated in favour of `recordTestRunHistory`, and `executionOrder="depends,defects"` warns that it cannot order by defects while that history is disabled. The two settings had in fact always contradicted each other — `defects` ordering needs the history `cacheResult="false"` was turning off, so it was inert — and the deliberate no-history choice is kept : `recordTestRunHistory="false"`, `executionOrder="depends"`. No test behaviour change ; `composer test` exits with `0` again.
